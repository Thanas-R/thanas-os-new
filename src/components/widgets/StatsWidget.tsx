...
export const StatsWidget = () => {
  ...
  const rings = [
    { label: 'Projects', value: 11, max: 20, color: 'hsl(0 100% 60%)', icon: FolderGit2 },
    { label: 'Stars', value: stats.stars, max: 50, color: 'hsl(130 70% 50%)', icon: Star },
    { label: 'LinkedIn', value: '40+', max: 100, color: 'hsl(210 100% 55%)', icon: Linkedin, isStatic: true },
  ];

  return (
    <div
      className="rounded-2xl p-4 shadow-macos-glass flex items-center gap-5 backdrop-blur-macos"
      style={{
        width: 352,
        ...
        <>
          ...
          <div className="relative w-20 h-20 flex-shrink-0">
            {rings.map((ring, idx) => {
              ...
              const numVal = ring.isStatic ? 40 : (ring.value as number);

...
export const Dock = () => {
  ...
  // Treat trash as part of the animated dock flow
  const visualItems = useMemo(
    () => [
      ...dockItems.map(item => ({ ...item, kind: 'app' as const })),
      { kind: 'trash' as const, appId: '__trash__' },
    ],
    [dockItems]
  );

  const trashIndex = visualItems.length - 1;
  ...
  const calculateTargetMagnification = useCallback(
    (mousePosition: number | null) => {
      if (mousePosition === null) {
        return visualItems.map(() => minScale);
      }

      return visualItems.map((_, index) => {
    ...
    [visualItems, baseSize, baseSpacing, effectWidth, maxScale, minScale]
  );
  ...
  useEffect(() => {
    const initialScales = visualItems.map(() => minScale);
    const initialPositions = calculatePositions(initialScales);
    setCurrentScales(initialScales);
    setCurrentPositions(initialPositions);
  }, [visualItems, calculatePositions, minScale, config]);
  ...
  const animateToTarget = useCallback(() => {
    ...
    const positionsNeedUpdate = currentPositions.some(
      (pos, index) => Math.abs(pos - targetPositions[index]) > 0.2
    );
  ...
  const baseContentWidth =
    currentPositions.length > 0
      ? Math.max(
          ...currentPositions.map((pos, index) => pos + (baseSize * currentScales[index]) / 2)
        )
      : visualItems.length * (baseSize + baseSpacing) - baseSpacing;

  const contentWidth = baseContentWidth;
  const padding = Math.max(8, baseSize * 0.12);

  // Separator placement between apps and trash
  const trashScale = currentScales[trashIndex] || 1;
  const trashPosition = currentPositions[trashIndex] || 0;
  const trashScaledSize = baseSize * trashScale;
  const separatorGap = baseSpacing * 1.5;
  const separatorWidth = 1;
  const separatorLeft = trashPosition - trashScaledSize / 2 - separatorGap - separatorWidth;
                ...
                >
                  <div
                    ...
                    style={{
                      ...
                      filter: `drop-shadow(0 ${Math.max(1, baseSize * 0.03)}px ${Math.max(
                        2,
                        baseSize * 0.06
                      )}px rgba(0,0,0,0.3))`,
                    }}
            ...
            return (
              ...
              >
                <div
                  ...
                  style={{
                    ...
                    filter: `drop-shadow(0 ${
                      scale > 1.2 ? Math.max(2, baseSize * 0.05) : Math.max(1, baseSize * 0.03)
                    }px ${
                      scale > 1.2 ? Math.max(4, baseSize * 0.1) : Math.max(2, baseSize * 0.06)
                ...
                {mouseX !== null && Math.abs(position - mouseX - padding) < 50 && (
                  <div
                    className={`absolute -top-12 backdrop-blur-macos-heavy bg-[hsl(var(--macos-glass))] px-3 py-1.5 rounded-lg text-sm font-medium whitespace-nowrap border border-[hsl(var(--macos-glass-border))] shadow-macos-glass ${

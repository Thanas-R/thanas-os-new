...
export const Dock = () => {
  ...
  const animateToTarget = useCallback(() => {
    ...
    const nextScales = currentScales.map((scale, index) => {
      const next = scale + (targetScales[index] - scale) * lerpFactor;
      if (Math.abs(next - targetScales[index]) > 0.001) {
        needsAnotherFrame = true;
      }
      return next;
    });

    const nextPositions = currentPositions.map((pos, index) => {
      return pos + (targetPositions[index] - pos) * lerpFactor;
    });
    ...
    if (needsAnotherFrame || mouseX !== null) {
  ...
  }, [animateToTarget, dockItems.length]);
  ...
  const handleMouseMove = useCallback(
    ...
    [padding, settings.reducedMotion]
  );
  ...
  const handleAppClick = (appId: string, index: number) => {
    if (iconRefs.current[index] && !settings.reducedMotion) {
      if (typeof window !== 'undefined' && window.gsap) {
        ...
        const bounceHeight =
          scalesRef.current[index] > 1.3 ? -baseSize * 0.2 : -baseSize * 0.15;
      ...
      >
        ...
        >
          {dockItems.map((item, index) => {
            ...
            const scale = scalesRef.current[index] || 1;
            ...
            return (
              ...
              >
                ...
                {mouseX !== null && Math.abs(position - mouseX) < 50 && (
                  <div

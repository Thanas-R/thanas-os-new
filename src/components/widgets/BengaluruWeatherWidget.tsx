...
import { BsFillMoonStarsFill, BsFillSunsetFill, BsFillSunriseFill, BsFillCloudMoonFill, BsCloudDrizzleFill, BsCloudLightningFill, BsCloudLightningRainFill } from 'react-icons/bs';
...
const WeatherGlyph = ({ code, isDay, size = 22, sunset, sunrise }: { code: number; isDay: number; size?: number; sunset?: boolean; sunrise?: boolean }) => {
...
export const BengaluruWeatherWidget = ({ className }: { className?: string }) => {
  ...
  useEffect(() => {
    const url = 'https://api.open-meteo.com/v1/forecast?latitude=12.9716&longitude=77.5946&current=temperature_2m,is_day,weather_code&hourly=temperature_2m,weather_code,is_day&daily=temperature_2m_max,temperature_2m_min,sunset,sunrise&timezone=Asia%2FKolkata&forecast_days=2';
    fetch(url).then(r => r.json()).then(j => {
      const cur = j.current; const daily = j.daily;
      setCurrent({
      ...
      for (let i = 0; i < 6; i++) {
        ...
        const hr = (h % 12) || 12;
        slots.push({ hour: `${hr}${ampm}`, temp: Math.round(j.hourly.temperature_2m[k]), code: j.hourly.weather_code[k], is_day: j.hourly.is_day[k] });
      }
    ...
    >
      <div className="px-4 pt-3 pb-2.5">
        <div className="flex items-start justify-between">
          <div>
            <div className="text-[14px]" style={{ fontWeight: 500, letterSpacing: '0.01em' }}>Bengaluru</div>
            <div className="leading-none mt-1" style={{ fontSize: 42, fontWeight: 400, letterSpacing: '-0.02em' }}>
              {current ? `${current.temp}°` : '—'}
            </div>
          </div>
          <div className="text-right flex flex-col items-end gap-1">
            {current && <WeatherGlyph code={current.weather_code} isDay={current.is_day} size={26} />}
            {current && (
              <>
                <div className="text-[12px]" style={{ fontWeight: 500 }}>{conditionLabel(current.weather_code, current.is_day)}</div>
                <div className="text-[11px] text-white/75" style={{ fontWeight: 400 }}>H:{current.high}° L:{current.low}°</div>
              </>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-6 gap-1 mt-3">
          {hours.map((h, i) => (
            <div key={i} className="flex flex-col items-center gap-1.5">
              <div className="text-[10px] text-white/80" style={{ fontWeight: 500 }}>{h.hour}</div>
              <WeatherGlyph code={h.code} isDay={h.is_day} size={18} sunset={h.isSunset} sunrise={h.isSunrise} />
              {!h.isSunset && !h.isSunrise ? (
                <div className="text-[12px]" style={{ fontWeight: 600 }}>{h.temp}°</div>
              ) : (
                <div className="text-[10px] text-white/75" style={{ fontWeight: 500 }}>{h.isSunset ? 'Set' : 'Rise'}</div>
              )}
            </div>
          ))}
        </div>
